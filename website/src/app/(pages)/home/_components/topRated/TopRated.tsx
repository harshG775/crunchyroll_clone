...
import { getTopRated} from "@/utils/AxiosTMDB";
...
export default async function TopRated() {
    try {
        const [movie, tv] = await Promise.all([
            getTopRated({mediaType:"movie"}).then((data) => data.data),
            getTopRated({mediaType:"tv"}).then((data) => data.data),
        ]);
        return (
            <section>
                <Tabs defaultValue="movie">
                    ...
                    <TabsContent value="movie" asChild>
                        <div className="min-h-96 lg:max-h-[1280px] overflow-y-auto grid gap-2 grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(180px,1fr))]">
                            {movie.results.map(
                                (item: TrendingMovieItemType) => (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            />
                                            <CardHeader className="p-2 sm:p-4 shadow-lg rounded-lg border absolute bottom-1 left-1 right-1 bg-card/50 backdrop-blur-md grid items-end">
